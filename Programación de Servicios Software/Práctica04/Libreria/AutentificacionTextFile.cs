namespace PSS.jgb347.Practica_05
{
    public class AutentificacionTextFile : IAutentificacion
    {
        ...
        public AutentificacionTextFile(string textFile, FormatoRegistro formatoRegistro, string finCampo)
        {
            ...
            if ((textFile != null) && File.Exists(textFile))
            {
                using (var fs = File.OpenRead(textFile))
                {
                    ...
                    while ((reg = srTextFile.ReadLine()) != null) //While para leer cada linea del archivo de texto     
                    {
        ...
        private string Codificar(IUsuarioView user) // convertir un objeto IUsuarioView en una cadena de texto
        {
            string cadena;
