...
public class Network<Vertex extends Comparable<Vertex>> implements Graph<Vertex>, Iterable<Vertex> {
	...
	public String toString() {
		...
		for (Entry<Vertex, HashMap<Vertex, Double>> entry : this.adjacencyList.entrySet()) {
			resultado.put(entry.getKey(), new TreeMap<>(entry.getValue()));
		}
	...
	private TreeMap<Vertex, Par<Vertex, Double>> Dijkstra(Vertex source, Vertex destination) {
		...
		for (Vertex v : this.adjacencyList.keySet()) {
			if (v.equals(source))
				...
				DS.put(v, new Par<>(isAdjacent(source, v) ? source : null,
						isAdjacent(source, v) ? getWeight(source, v) : INFINITY));
				V_minus_S.add(v);
			}
	...
	public ArrayList<Par<Vertex, Double>> getDijkstra(Vertex source, Vertex destination) {// A veces cae en los examenes
		...
		if (this.salidaDijkstra.get(destination).getKey() == null)
			return null;
