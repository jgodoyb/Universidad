...
public class ArbolBinarioBasico<T extends Comparable<T>> {
	...
	//Método toString() -> recorrido IND
	@Override
	public String toString() {
		if (this.root == null) return "";
		String result = toString(this.root);
		return result.isEmpty() ? "" : "[" + result.substring(0, result.length()-2) + "]"; //Un poco chapuza, ¿verdad?
	}
	...
	private void toArray(Nodo<T> current, ArrayList<T> arr) {
		if(current.izq != null) {
			toArray(current.izq, arr);
		}
		arr.add(current.dato);
		if(current.dch != null) {
			toArray(current.dch, arr);
		}
	}
	
	//Método toArrayWithLevel() -> Igual que el anterior, pero solo devuelve las referencias a los nodos que están situados en un determinado nivel especificado como parámetro de entrada
	...
	private void toArrayWithLevel(Nodo<T> current, ArrayList<T> arr, int level, int levelCurrent) {
		if(current.izq != null) {
			toArrayWithLevel(current.izq, arr, level, levelCurrent + 1);
		}
		if(level == levelCurrent) {
			arr.add(current.dato);
		}
		if(current.dch != null) {
			toArrayWithLevel(current.dch, arr, level, levelCurrent + 1);
		}
	}
	...
	private int getNumHojas(Nodo<T> current) {
		...
		if(current.izq == null && current.dch == null) {
			return 1;
		}
		return getNumHojas(current.izq) + getNumHojas(current.dch);
	}
	
	//Método getNumNodosInternos()
	...
	private int getNumNodosInternos(Nodo<T> current) {
		...
		int count = current.izq != null || current.dch != null ? 1 : 0;
		return count + getNumNodosInternos(current.izq) + getNumNodosInternos(current.dch);
	}
