...
void ParametersError()
{
    ...
    puts("\t  -o <file> [Image]>	");
    puts("\t..-g  (gray colors) [default colored without -g]");
    exit(0);
}
...
// Move the number of iterations to RGB using a palette.
{
...
int main(int argc, char **argv)
{
    ...
    // InGray si la imagen en tonos de gris y !Ingray si el color.
    if (InGray)
    {
        // GetMem for ppRed, ppGreen and ppBlue. It puts data to zeroes
        ppGray = (char **)GetMem2D(Rows, Cols, sizeof(char), "Main:ppGray");
    }

    if (!InGray)
    {
        // Descomenta esta sección para asignar memoria para ppRed, ppGreen y ppBlue
    ...
    for (int i = 0; i < Rows; i++)
        for (int j = 0; j < Cols; j++)
        {
            ...
            if (!InGray)
            {
                // Descomenta esta sección para actualizar las matrices de color RGB
    ...
    if (!InGray)
    {
        // Descomenta esta sección para guardar los colores RGB en archivos RAW
        for (int i=0;i<Rows;i++){
		 for (int j=0;j<Cols;j++)
		     {
    ...
    system(Command);
    // eog from gnome visualiza la imagen cuando cambia
