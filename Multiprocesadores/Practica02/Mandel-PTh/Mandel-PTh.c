...
//Metodo el cual nos permite inicializar todos y cada uno de los colores
...
/*Realiza un conjunto de operaciones para poder determinar el valor que va en un solo pixel*/
 ...
 while (*(pMyData->pRowToProcess)< Rows)
       {
        ...
        //TODO: lock mutex to get MyRow -> Row to process
           ...
           pthread_mutex_t *pth_RowToProcess_mutex = pMyData->pth_RowToProcess_mutex;
           pthread_mutex_lock(pth_RowToProcess_mutex);
           
        if (*(pMyData->pRowToProcess) < pMyData->Rows)
           {
               
            //TODO: Set MyRow
            MyRow = *pMyData->pRowToProcess;
            //TODO: Set MyChunkSize without reach Rows
               MyChunckSize = *pMyData->pChunckSize;
              
               if(MyRow+MyChunckSize>pMyData->Rows){
                   MyChunckSize = pMyData->Rows - MyRow;
                   printf("Thread %d - %d = %d\n", 
                pMyData->Rows, MyRow, MyChunckSize);
               }
        ...
        //TODO: unlock  mutex
            pthread_mutex_unlock(pth_RowToProcess_mutex);
...
int main(int argc, char **argv)
{
 ...
 //Get mem for threads
 pThreads = (pthread_t *) GetMem(NThreads+1, sizeof(pthread_t), 
                                 "Main: pThreads");
 ...
 //TODO: Run Th_Mandel() here for the main thread
    pthread_create(&pThreads[0], NULL, Th_Mandel, (void *) &pTh_Args[0]);
