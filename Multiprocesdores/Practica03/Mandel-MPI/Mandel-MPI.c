...
int main(int argc, char **argv)
{
 ...
 if (MyRank==0)	//Task 0------------------------
 ...
 else			//Task != 0------------------------
	{
	 ...
	 MPI_Recv(&RowToDo,1,MPI_INT,MPI_ANY_SOURCE,TagRowToDo,MPI_COMM_WORLD,&Stat);
	 #if (PRINT>=1)
	 ...
	 while (RowToDo < Rows)
	      {
           ...
           MPI_Send(&RowDone,1,MPI_INT,Stat.MPI_SOURCE,TagRowDone,MPI_COMM_WORLD);
           #if (PRINT>=1)
 		   printf("Taks %d send. RowDone %d to Task 0,\n",
 		           MyRank,RowDone);
 		    #endif  
 		   //TODO: Send to task 0 calculated RGB 
           MPI_Send(&ppRed[0][0],Cols,MPI_CHAR,Stat.MPI_SOURCE,TagRowRed,MPI_COMM_WORLD);
           MPI_Send(&ppGreen[0][0],Cols,MPI_CHAR,Stat.MPI_SOURCE,TagRowGreen,MPI_COMM_WORLD);
           MPI_Send(&ppBlue[0][0],Cols,MPI_CHAR,Stat.MPI_SOURCE,TagRowBlue,MPI_COMM_WORLD);
           #if (PRINT>=1)
